...
- each parameter must be typed in a #DECLARE
- Use `:=` for assignment, not `=`.
- Do not use ; for end of code lines.

## **Components**
Components are stored inside `Project/Components`. A component is itself a 4D project that can share methods, classes and forms with the main project.
...
- Host methods can be accessed from the component project if they are declared as `Shared by components and host project`
...
- A function that starts with "_" is considered private to the class.
...
- Placed above the class constructor and defined as:
  ...
  ```
- A property that starts with "_" is considered private to the class.
...
### **Singleton class (only available in 4D 21 and beyond)**
...
- Must be suffixed with `__UnitTest`.
