...
If (__STACK.length>0)
	...
	$vt_buffer:=$vt_buffer+"[p"+String:C10(Current process:C322; "000")
	...
	$vt_buffer:=$vt_buffer+"]: ###### CALLING CHAIN DUMP"+Char:C90(Carriage return:K15:38)
	
	// Output the calling chain
	C_LONGINT:C283($i)
	For ($i; 0; __STACK.length-1)
		$vt_buffer:=$vt_buffer+" Lvl "+String:C10($i; "##00")+": "+("   "*($i))+__STACK[$i].tag
		If (__STACK[$i].extraText#"")
			$vt_buffer:=$vt_buffer+" -- "+__STACK[$i].extraText
		End if 
		$vt_buffer:=$vt_buffer+Char:C90(Carriage return:K15:38)
	End for 
	
	// Output our ERROR # 
	If ($err#0)
		$vt_buffer:=$vt_buffer+" Lvl "+String:C10(__STACK.length; "##00")+": "+("   "*(__STACK.length))+"above method triggered err #"+String:C10($err)+Char:C90(Carriage return:K15:38)
	End if 
